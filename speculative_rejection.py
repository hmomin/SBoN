...
class SpeculativeRejection(Generator):
        ...
        while current_length < max_all_tokens:
            ...
            try:
                if isinstance(self.generation_model, LLM):
                    ...
                    # while gen_len > 1:
                    #     batch_size = int(batch_size * 0.9)
                    #     gen_len =  self.generation_model.get_gen_len(batch_size=batch_size, cur_len=cur_len)
                    assert gen_len > 0

                    if (
                        batch_size < 5 and self.draft_model is not None
                    ):  # use spec dec to speed up
                        gamma = 2
                        gen_len = min(
                            gen_len,
                            self.generation_model.max_tokens
                            - 1
                            - gamma
                            - batch_encoding.input_ids.shape[1],
                        )
                        partial_generation = self.generation_model.speculative_decoding(
                        ...
                        # print('[input]:', batch_encoding, batch_encoding.shape)
                        ...
                        # print('[gen]:', partial_generation, partial_generation.shape)
                        # print(self.generation_model.decode(input_ids=partial_generation[:4]))
                else:
                    # print('[input]:', batch_encoding.input_ids, batch_encoding.input_ids.shape)
            ...
            self.clock.start()
        # NOTE: this may create several duplicate trajectories, but that's okay
        self.trajectories = (
    ...
    def perform_speculative_rejection(
        ...
        # print(len(trajectories_to_rank), len(previous_finished_trajectories), len(current_trajectories))
